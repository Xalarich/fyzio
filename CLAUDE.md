...
**Cookie consent**: GDPR consent is hand-rolled (no library). `app/composables/useCookieConsent.ts` holds shared state via `useState` + `localStorage` (key `fyzio-cookie-consent`, shape `{necessary:true, maps:bool, analytics:bool, ts}`); `hydrate()` runs client-side `onMounted` (SSR/prerender always renders the "not decided, all optional off" state, so static HTML never embeds third-party iframes nor loads analytics before consent). `app/components/CookieConsent.client.vue` is the banner + category-settings panel (mounted once in `default.vue`). Two optional categories exist: **`maps`** (the Google Maps iframe on `kontakt.vue` renders only behind `has('maps')`, otherwise a "Zobrazit mapu" placeholder) and **`analytics`** (Google Analytics — see below). Footer has a "Nastavení cookies" button (`openSettings()`) plus a link to `/zasady-cookies` (a noindex cookie-policy page that can also toggle just the analytics category). To add another optional category: extend `ConsentState` + `defaultState` + `hydrate`, add a toggle row in the component, gate the feature behind `has('<cat>')`. Design doc: `docs/superpowers/specs/2026-06-23-cookie-consent-design.md`.

**Map on `/kontakt` — never go back to Mapy.cz.** The map was `https://frame.mapy.cz/s/mobunosopa`, a *shortened share link*. `frame.mapy.cz` has been retired: that URL now takes **4 redirects** into `mapy.com/cs/zakladni?…&frame=1`, which is the **full mapy.com single-page app (~171 requests) loaded inside the iframe**. In a normal Chrome profile that pins the renderer — the map stays white or paints only a corner, the page freezes, and nothing can be panned. Verified: the same URL opened as a *top-level page* works fine, so Mapy.cz is not blocked; it is the embed that fails. Replaced with a keyless Google Maps embed (`https://www.google.com/maps?q=<adresa>&z=16&output=embed`), which renders instantly and produced **zero long tasks** on the parent thread over 9 s. It is deliberately **not** grayscale — the map shows in colour. Any Mapy.cz/Mapy.com iframe is a regression; if the provider ever changes again, also update the user-facing copy in `CookieConsent.client.vue`, `zasady-cookies.vue` and `ochrana-udaju.vue`, which all name the provider.
